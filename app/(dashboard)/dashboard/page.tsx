...
export default function DashboardPage() {
    ...
    const kpis = [
        { label: 'Novos Pacientes', value: data.kpis.newPatients.value, trend: data.kpis.newPatients.trend, icon: Users, color: 'text-brand-primary' },
        { label: 'Consultas Realizadas', value: data.kpis.completedAppointments.value, trend: data.kpis.completedAppointments.trend, icon: CalendarCheck, color: 'text-blue-400' },
        { label: 'Tratamentos Indicados', value: data.kpis.indicatedTreatments.value, trend: data.kpis.indicatedTreatments.trend, icon: Target, color: 'text-orange-400' },
        { label: 'Faturamento Mensal', value: formatCurrency(data.kpis.monthlyRevenue.value), trend: data.kpis.monthlyRevenue.trend, icon: CircleDollarSign, color: 'text-emerald-400' },
    ]
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {kpis.map((kpi, idx) => (
                    <div key={idx} className="card p-5 group hover:glow-border transition-all duration-300">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    <div className="card p-6 bg-gradient-to-br from-bg-surface to-bg-elevated border-brand-primary/20">
                        ...
                        <button className="w-full py-2 bg-brand-primary text-bg-app font-semibold rounded-sm hover:bg-brand-accent transition-all text-sm">
                            Reativar Pacientes
                        </button>
                    </div>
                    ...
                    <div className="card p-6">
                        ...
                        <button className="w-full mt-4 text-xs font-semibold text-text-muted hover:text-brand-primary transition-colors py-2 border border-bg-border rounded-sm">
                            Ver agenda completa
                        </button>
                    </div>
                </div>
